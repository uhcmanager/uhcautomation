...
public class CommandHelp extends UHCCommand {
    public CommandHelp() {
        name = "help";
        adminOnly = false;
    }
    ...
    public BaseComponent[] buildHelpMsg(boolean hasPerm) {
        ...
        Arrays.stream(CommandHandler.getOPTIONS_ALIASES()).forEach(optionAliasJoiner::add);
        ...
        Arrays.stream(CommandHandler.getREGISTER_ALIASES()).forEach(registerAliasJoiner::add);
        ...
        Arrays.stream(CommandHandler.getUNREGISTER_ALIASES()).forEach(unregsiterAliasJoiner::add);
